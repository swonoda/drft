...
test("カーソル位置をプレビュー用マーカーへ変換する", () => {
  const source = "題名\n\n本文です。";
  const html = renderPreviewDocument(source, source.indexOf("で"));
  assert.match(html, /本文<span class="preview-caret">で<\/span>す/);
  assert.equal((html.match(/preview-caret/g) || []).length, 1);
});
test("プレビューのカーソルは次の一文字を反転し、文末では直前を反転する", () => {
  const source = "題名\n\n本文";
  assert.match(
    renderPreviewDocument(source, source.indexOf("本")),
    /<span class="preview-caret">本<\/span>文/,
  );
  assert.match(
    renderPreviewDocument(source, source.length),
    /本<span class="preview-caret">文<\/span>/,
  );
});
test("タイトルのカーソルは本文先頭へ二重表示しない", () => {
  const source = "題名\n\n本文";
  const html = renderPreviewDocument(source, 1);
  assert.match(html, /<h1>題<span class="preview-caret">名<\/span><\/h1>/);
  assert.doesNotMatch(html, /<p><span class="preview-caret">本<\/span>/);
  assert.equal((html.match(/preview-caret/g) || []).length, 1);
});
test("複数行の節でもカーソル位置を正しく変換する", () => {
  ...
  assert.match(html, /三行<span class="preview-caret"/);
  assert.equal((html.match(/preview-caret/g) || []).length, 1);
});
test("章と節の間の空行では次の要素へカーソルを対応させる", () => {
  const source = "題名\n\n\n第一章\n\n本文";
  const html = renderPreviewDocument(source, source.indexOf("第一章") - 1);
  assert.match(html, /<h2><span class="preview-caret"/);
  assert.equal((html.match(/preview-caret/g) || []).length, 1);
});
