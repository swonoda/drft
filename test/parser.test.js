...
test("修正マークを一覧化し、プレビューと文字数から除外する", () => {
  ...
  assert.match(html, /<span class="preview-fix-sentence">白熊が歩く。<\/span>/);
  assert.equal(manuscriptCharacterCount(source), 8);
});
test("修正マークに対応する文だけをプレビュー用に識別する", () => {
  const source = "題名\n\n最初の文。直す文#fix[表現を見直す]。最後の文。";
  const html = renderPreviewDocument(source, source.indexOf("直す文") + 2);
  assert.match(
    html,
    /最初の文。<span class="preview-fix-sentence"><span class="preview-highlight"[^>]*>直す文。<\/span><\/span>最後の文。/,
  );
  assert.equal((html.match(/preview-fix-sentence/g) || []).length, 1);
});
test("文頭の修正マークは直後の文をプレビュー用に識別する", () => {
  const source = "題名\n\n前の文。#fix[]直す文？　次の文。";
  const html = renderPreviewDocument(source);
  assert.match(
    html,
    /前の文。<span class="preview-fix-sentence">直す文？<\/span>　次の文。/,
  );
});
test("文末と次の文頭にある修正マークを別々に識別する", () => {
  const source = "題名\n\n前の文#fix[]。#fix[]次の文？　最後の文。";
  const html = renderPreviewDocument(source);
  assert.match(
    html,
    /<span class="preview-fix-sentence">前の文。<\/span><span class="preview-fix-sentence">次の文？<\/span>　最後の文。/,
  );
  assert.equal((html.match(/preview-fix-sentence/g) || []).length, 2);
});
