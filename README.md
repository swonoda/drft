...
執筆は入力しやすい横書きで。必要なときだけ縦書きプレビューを開き、読者に近い状態で文章を確認できます。原稿はプレーンなTXT、作品専用の辞書はMarkdownで保存するため、独自形式に閉じ込められず、ほかの執筆ツールとも併用できます。

<!-- スクリーンショットを追加したら、次のコメントを画像パスに置き換えてください。 -->
...
- **必要なときだけ開く縦書きプレビュー**  
  横書きエディタと重ならない分割表示。カーソル位置とページを同期し、禁則処理と一行の文字数指定に対応します。

- **章と節を見渡せる構成ビュー**  
  原稿内の空行から章と節を読み取り、一覧から本文へジャンプできます。節はドラッグ＆ドロップで並べ替えられます。

- **青空文庫記法・カクヨム記法**  
  ルビとゴマ傍点を縦書きプレビューとPDFへ反映します。
...
- **UTF-8・Shift_JISに対応**  
  TXTを開くと文字コードを自動判定します。現在の文字コードは画面下部に表示され、クリックして保存形式を切り替えられます。

- **右綴じ見開きPDF**  
  本文先頭を最初の見開きの左側から開始し、右綴じの見開きへ面付けして出力します。

## 基本的な使い方

1. 「ファイル」→「新規作成」または「開く」で原稿を用意します。
2. 横書きのエディタで本文を執筆します。変更は保存後、自動保存されます。
3. 右上のプレビューアイコンから縦書き表示を開き、読み心地を確認します。
4. 原稿を開くと、同じフォルダの `原稿名_辞書.md` を作品専用辞書として利用できます。
...
辞書は、開いているTXTと同じフォルダの `原稿名_辞書.md` に保存されます。DRFTが管理しないファイルやサブフォルダは変更しません。以前の `辞書.md` も読み込めますが、次の保存から原稿ごとの名前になります。
